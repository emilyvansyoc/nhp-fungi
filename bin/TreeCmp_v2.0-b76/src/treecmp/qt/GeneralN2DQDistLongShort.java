...
public class GeneralN2DQDistLongShort extends Distance{
  ...
  protected long calcDistance(Tree gt1, Tree gt2) {
	...
	for (int pos1 = 0; pos1 < outedges1.length; pos1++) { //O(degree of current node in t1)
	  for (int pos2 = 0; pos2 < outedges2.length; pos2++) { //O(degree of current node in t2)
	    ...
	    fixed_t1_allout_t2[pos1] +=
	      (long)intersect[outedges1[pos1].getBackEdgeId()][outedges2[pos2].getId()] *
	      (long)intersect[outedges1[pos1].getId()][outedges2[pos2].getId()];

	    fixed_t2_allout_t1[pos2] +=
	      (long)intersect[outedges1[pos1].getId()][outedges2[pos2].getBackEdgeId()] *
	      (long)intersect[outedges1[pos1].getId()][outedges2[pos2].getId()];
	  }
	}
	...
	for (int pos1 = 0; pos1 < outedges1.length; pos1++) { //O(degree of current node in t1)
	  for (int row = 0; row < outedges1.length; row++) { //O(degree of current node in t1)
		...
		diff_innersums[pos1][row] +=
		  (long)intersect[outedges1[row].getId()][outedges2[col].getId()] *
		  (long)intersect[outedges1[pos1].getId()][outedges2[col].getId()];
	      }
	    }
	  }
	}
	...
	for (int pos1 = 0; pos1 < outedges1.length; pos1++) { //O(degree of current node in t1)
	  for (int pos2 = 0; pos2 < outedges2.length; pos2++) { //O(degree of current node in t2)
	    ...
	    diff +=
	      (long)intersect[outedges1[pos1].getId()][outedges2[pos2].getId()] *
	      ((long)intersect[outedges1[pos1].getBackEdgeId()][outedges2[pos2].getBackEdgeId()] *
	      (long)intersect[outedges1[pos1].getBackEdgeId()][outedges2[pos2].getId()] *
	      (long)intersect[outedges1[pos1].getId()][outedges2[pos2].getBackEdgeId()] -
	      (long)intersect[outedges1[pos1].getBackEdgeId()][outedges2[pos2].getId()] *
	      (fixed_t1_allout_t2[pos1] -
		...
		diff +=
		  (long)intersect[outedges1[pos1].getId()][outedges2[pos2].getId()] *
		  (long)intersect[outedges1[pos3].getId()][outedges2[pos2].getId()] *
    ...
    long dist = bq1 + bq2 - 2 * shared - diff;
    return dist;
  }
