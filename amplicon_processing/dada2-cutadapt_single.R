...
fnFs <- sort(list.files(path_to_files, pattern = ".fastq", full.names = TRUE))
...
trim <- filterAndTrim(fnFs, fnFs.filtN, maxN = 0, multithread = TRUE, verbose = TRUE)
...
tryCatch(
  {
    ...
    # remove empty files
    empties <- rownames(as.data.frame(trim) %>% filter(reads.out == 0))
    ...
    for (i in seq_along(fnFs.filtN)) {
      system2(cutadapt, args = c(
        "--quiet", R1.flags, "-n", 2, # -n 2 required to remove FWD and REV from reads
